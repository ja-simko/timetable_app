...
def build_timetable_df():
    ...
    stop_times_routes['main_station'] = stop_times_routes['stop_id'].str.extract(r"^(U\d+)[ZS]", expand=False)

    timetable = stop_times_routes[
        ['trip_id', 'stop_id', 'main_station', 'departure_time', 'arrival_time',
...
def build_edges():
    ...
    for row in timetable.itertuples(index=False):
        ...
        dep_time = timedelta(seconds = prev_row.departure_time) 
        arr_time = timedelta(seconds = row.arrival_time)
        shift = timedelta(days = 0)

        edges[out_node][in_node].append((dep_time, arr_time, row.trip_id, shift))

        if dep_time >= timedelta(hours = 24):
            new_dep_time = dep_time - timedelta(hours = 24)
            new_arr_time = arr_time - timedelta(hours = 24)
            shift = timedelta(days = -1)

        elif dep_time < timedelta(hours = 6):
            new_dep_time = dep_time + timedelta(hours = 24)
            new_arr_time = arr_time + timedelta(hours = 24)
            shift = timedelta(days = +1)
